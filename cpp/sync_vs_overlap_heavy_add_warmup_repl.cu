...
float experiment_grouped_ops_order(bool overlap, int N, int iterations, int cycles) {
    ...
    std::cout << "cuda time (" << (overlap ? "Overlapping" : "Synchronous") << "): " << elapse << " ms\n";
...
int main() {
    const int replication_trials = 10;
    const int warmup = 5;
    ...
    for (int i = 0; i < warmup; i++) {
        ...
        float sync = experiment_grouped_ops_order(false, warmup_size, iterations, best_cycles);

        std::cout << "Running overlapping version...\n";
        float async = experiment_grouped_ops_order(true, warmup_size, iterations, best_cycles);
    ...
    for (int i = 0; i < experiments; i++) {
        const int N = base;
        base *= 10;
        for (int j = 0; j < replication_trials; j++) {
            std::cout << "==============================" << std::endl
                    << "N = " << N
                    << ", #it = " << iterations
                    << ", cycles = " << best_cycles
